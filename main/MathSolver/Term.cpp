...
std::set<Variable> Term::listVariables(bool recursive) const {
        ...
        if (std::holds_alternative<Variable>(part.m_part)) {
            ...
            if (part.m_inverse) {
                ...
                for (const Term& term1 : terms) {
                    newTerms.push_back(term1 / std::get<Expression>(part.m_part).simplify());
                }
                terms = newTerms;
                continue;
            }
...
std::string Term::toString(bool ignoreNegativeAmount) const {
    ...
    auto vars = listVariables(false);
    for (const auto& var : vars) {
        int count = countOccurences(var);
        if (count < 0)
            output += "/";
        output += var.toString();
        if (std::abs(count) > 1) {
            output += ESPIO::intToUpperNum(count);
        }
    }
    for (const InversablePart& part : m_parts) {
        if (std::holds_alternative<Expression>(part.m_part))
...
Term Term::crushOppositeSame() const {
    ...
    for (int i = 0; i < parts.size(); i++) {
        ...
        if (std::holds_alternative<Expression>(part1.m_part))
            continue;
        for (int j = 0; j < parts.size(); j++) {
            InversablePart part2 = parts.at(j);
            if (std::holds_alternative<Expression>(part2.m_part))
                continue;
