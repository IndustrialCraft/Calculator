...
#define LCD_FUNCTION_SET_4BIT 0x2C  // 4-bit data, 2-line display, 5 x 7 font
#define LCD_SET_CURSOR 0x80         // set cursor position
#define LCD_UPLOAD_CHAR 0x40
...
static esp_err_t I2C_init(void) {
    i2c_config_t conf = {.mode = I2C_MODE_MASTER, .sda_io_num = SDA_pin, .scl_io_num = SCL_pin, .sda_pullup_en = GPIO_PULLUP_ENABLE, .scl_pullup_en = GPIO_PULLUP_ENABLE, .master.clk_speed = 100000};
...
void LCD_setCursorVisible(bool visible) {
    LCD_writeByte(LCD_DISPLAY_ON | (visible ? 0x02 : 0), LCD_COMMAND);
}
...
void LCD_uploadCharacter(uint8_t location, uint8_t texture[]) {
    location &= 0x7;
    LCD_writeByte(LCD_UPLOAD_CHAR | (location << 3), LCD_COMMAND);
    for (int i = 0; i < 8; i++) {
        LCD_writeByte(texture[i], 1);
    }
}
